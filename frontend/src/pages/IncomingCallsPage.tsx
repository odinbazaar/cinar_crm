                ...
                <div className="flex items-center gap-2">
                    <div className="flex items-center bg-gray-100 p-1 rounded-xl border border-gray-200 shadow-inner">
                        ...
                        <button
                            ...
                            className={`p-2 rounded-lg transition-all ${viewMode === 'table' ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                            title="Liste/Excel Görünümü"
                        >
                    ...
                    <div className="hidden lg:block text-sm text-gray-500 bg-gray-50 px-3 py-2 rounded-lg border border-gray-100">
                        {filteredCalls.length} / {incomingCalls.length} kayıt
                    </div>
                </div>
            </div>
                        ...
                        <table className="w-full text-left border-collapse min-w-[1000px]">
                            <thead>
                                <tr className="bg-gray-50/50 border-b border-gray-100">
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Firma Adı</th>
                                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Yetkili</th>
                                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Telefon</th>
                                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Aranan Hat</th>
                                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Talep</th>
                                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">İşlemler</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {filteredCalls.map((call) => (
                                    <tr key={call.id} className="hover:bg-primary-50/30 transition-colors group">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase ${call.status === 'pending' ? 'bg-yellow-100 text-yellow-700' :
                                                call.status === 'contacted' ? 'bg-blue-100 text-blue-700' :
                                                    call.status === 'converted' ? 'bg-green-100 text-green-700' :
                                                        'bg-gray-100 text-gray-700'
                                                }`}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center gap-2 font-mono text-sm font-bold text-gray-700">
                        ...
                        <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden">
                            ...
                            <div className="p-8 space-y-5 max-h-[70vh] overflow-y-auto">
                                ...
                                <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100 space-y-4">
                                    <div className="flex items-center gap-2 text-orange-600 font-black text-[10px] uppercase tracking-widest mb-2 ml-1">
                                    ...
                                    <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-3">
                                        <textarea
                                            ...
                                            className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 text-sm focus:ring-2 focus:ring-orange-500 min-h-[80px]"
                                        />
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1">Hatırlatma Tarihi</label>
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full pl-9 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-900 focus:ring-2 focus:ring-orange-500"
                                                    />
                                                </div>
                                            </div>
                                            <div>
                                                <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1">Saat</label>
                                                <div className="relative">
                                            ...
                                            <label className="flex items-center gap-2 cursor-pointer group">
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        className="sr-only"
                                                    />
                                                    <div className={`w-8 h-4 rounded-full transition-colors ${noteInput.repeat ? 'bg-orange-500' : 'bg-gray-200'}`}></div>
                                                ...
                                                <span className="text-[10px] font-bold text-gray-400 uppercase group-hover:text-gray-600">Tekrarlı</span>
                                            </label>
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    ...
                                                    const currentNotes = newCallForm.notes ? (newCallForm.notes.startsWith('[') ? JSON.parse(newCallForm.notes) : []) : [];
                                                    const updatedNotes = JSON.stringify([...currentNotes, newNote]);
                                                    setNewCallForm({ ...newCallForm, notes: updatedNotes });
                                                    setNoteInput({ ...noteInput, content: '', reminderDate: '', repeat: false });
                                                }
                                                }
                                                className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-[10px] font-black uppercase rounded-lg transition-all flex items-center gap-2"
                                            >
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between px-1">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                />
                                                {noteFilterDate && (
                                                    <button onClick={() => setNoteFilterDate('')} className="text-[10px] text-orange-600 font-bold hover:underline">Temizle</button>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="max-h-[200px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                                            {(() => {
                                                const notesStr = newCallForm.notes || '[]';
                                                let notesArr = [];
                                                try {
                                                ...
                                                if (filtered.length === 0) {
                                                    return <div className="text-center py-6 text-gray-400 text-[10px] uppercase font-bold italic">Not bulunamadı.</div>;
                                                }

                                                return [...filtered].sort((a: any, b: any) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()).map((note: any) => (
                                                    <div key={note.id} className="p-3 bg-white border border-gray-100 rounded-2xl group relative shadow-sm">
                                                        <div className="flex items-center gap-2 mb-1.5">
                                                            <span className="px-1.5 py-0.5 bg-gray-100 text-gray-500 text-[9px] font-bold rounded uppercase">
                                                                {note.date}
                                                            </span>
                                                            {note.reminderDate && (
                                                                <span className={`px-1.5 py-0.5 ${note.isReminded ? 'bg-gray-100 text-gray-400' : 'bg-red-50 text-red-500'} text-[9px] font-bold rounded flex items-center gap-1 border border-red-100`}>
